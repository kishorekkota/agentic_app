...
class Client_graph():
    ...
    def __init__(self):
        ...
        self.connection_string=os.environ["STORAGE_CONNECTION"]
        self.blob_path=os.environ['CLIENT_DEMOGRAPHICS_BLOB_PATH']
        self.categories_list=['overtime','sick leave','wage']
        self.categories = ', '.join(self.categories_list)
        fs = AzureBlobFileSystem(connection_string=self.connection_string)
        with fs.open(self.blob_path, 'rb') as f:
            self.client_demographics = pd.read_csv(f)
